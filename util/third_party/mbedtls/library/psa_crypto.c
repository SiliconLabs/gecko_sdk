                          ...
                          mbedtls_svc_key_id_t *target_key)
{
    ...
    *target_key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    if (!source_key_is_unconditionally_copyable &&
        PSA_KEY_LIFETIME_GET_LOCATION(target_slot->attr.lifetime) !=
    ...
    if (psa_key_lifetime_is_external(actual_attributes.core.lifetime)) {
        ...
        if (psa_key_lifetime_is_external(source_slot->attr.lifetime)) {
          // Silicon Labs's opaque driver for key copying is only able to
          // import transparent keys.
          status = PSA_ERROR_NOT_SUPPORTED;
          goto exit;
        }
