 ...
 * \param[inout] expected_size  On success, a byte size large enough to contain
                                                    ...
                                                    size_t* storage_size )
{
    if( psa_key_lifetime_is_external( attributes->core.lifetime ) )
    {
        ...
        return psa_driver_wrapper_get_key_buffer_size( attributes, storage_size );
#else
        return( PSA_ERROR_NOT_SUPPORTED );
#endif
    }
    else
    {
        ...
        {
            size_t expected_size = PSA_EXPORT_KEY_OUTPUT_SIZE( attributes->core.type, attributes->core.bits );
            if( expected_size == 0 )
                return( PSA_ERROR_NOT_SUPPORTED );
                             ...
                             mbedtls_svc_key_id_t *key )
{
    ...
    *key = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = psa_calculate_key_storage_size( attributes, &storage_size );
    if( status != PSA_SUCCESS )
        goto exit;
                                     ...
                                     size_t *output_length )
{
    ...
    *output_length = 0;
    ...
    status = psa_driver_wrapper_asymmetric_encrypt( slot,
                                                    alg,
                                                    input,
                                                    input_length,
                                                    salt,
                                     ...
                                     size_t *output_length )
{
    ...
    *output_length = 0;
    ...
    status = psa_driver_wrapper_asymmetric_decrypt( slot,
                                                    alg,
                                                    input,
                                                    input_length,
                                                    salt,
    ...
    mbedtls_svc_key_id_t *key_out )
{
    ...
    *key_out = MBEDTLS_SVC_KEY_ID_INIT;
    ...
    status = psa_calculate_key_storage_size( key_out_attributes, &storage_size );
    if( status != PSA_SUCCESS )
        goto exit;
    ...
    {
        psa_key_attributes_t key_in_attributes = {
          .core = input_key_slot->attr
        };
    ...
    #if defined(PSA_WANT_ALG_PBKDF2) \
        && (defined(SEMAILBOX_PRESENT) \
        && (_SILICON_LABS_SECURITY_FEATURE == _SILICON_LABS_SECURITY_FEATURE_VAULT))
        {
            /* Insert call to PBKDF2 driver once available. */
            status = PSA_ERROR_NOT_SUPPORTED;
        }
...
exit:
    ...
    unlock_status = psa_unlock_key_slot( input_key_slot );
...
#define PSA_KEY_AGREEMENT_MAX_SHARED_SECRET_SIZE PSA_KEY_EXPORT_ECC_PUBLIC_KEY_MAX_SIZE(PSA_VENDOR_ECC_MAX_CURVE_BITS)
                                                    ...
                                                    size_t *shared_secret_length )
{
    ...
    status = psa_driver_wrapper_key_agreement( alg,
                                               private_key,
                                               peer_key,
                                               peer_key_length,
                                               shared_secret,
                                               ...
                                               shared_secret_length );
    if( status != PSA_ERROR_NOT_SUPPORTED ||
        psa_key_lifetime_is_external( private_key->attr.lifetime ) )
        return status;
...
psa_status_t psa_crypto_init( void )
{
    ...
    if( status != PSA_ERROR_NOT_SUPPORTED && status != PSA_SUCCESS )
        goto exit;
