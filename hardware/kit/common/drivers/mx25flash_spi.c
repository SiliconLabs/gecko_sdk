...
#if defined(MX25_USART)
#include "em_usart.h"

#elif defined(MX25_EUSART)
#include "em_eusart.h"
#endif
...
/* FIXME TODO complete if/else for USART EUSART and see if naive port just works (TM) */
...
#elif defined(MX25_EUSART)
   EUSART_SpiInit_TypeDef spi_init = EUSART_SPI_MASTER_INIT_DEFAULT_HF;
   EUSART_SpiAdvancedInit_TypeDef adv_init = EUSART_SPI_ADVANCED_INIT_DEFAULT;
   spi_init.bitRate = MX25_BAUDRATE;
   spi_init.advancedSettings = &adv_init;
   adv_init.msbFirst = true;

   CMU_ClockSelectSet(cmuClock_EM01GRPCCLK, cmuSelect_HFRCODPLL);
   CMU_ClockEnable( MX25_EUSART_SCLK, true );

   // EUSART_BaudrateSet(MX25_EUSART, 0, MX25_BAUDRATE);
   /* Initialize the EUSART */
   EUSART_SpiInit(MX25_EUSART, &spi_init);
#endif
...
#endif
#elif defined(MX25_EUSART)
   MX25_EUSART_ROUTE.SCLKROUTE  = ( (MX25_PORT_SCLK << _GPIO_EUSART_SCLKROUTE_PORT_SHIFT)
                                  | (MX25_PIN_SCLK  << _GPIO_EUSART_SCLKROUTE_PIN_SHIFT) );
   MX25_EUSART_ROUTE.RXROUTE   = ( (MX25_PORT_MISO << _GPIO_EUSART_RXROUTE_PORT_SHIFT)
                                  | (MX25_PIN_MISO  << _GPIO_EUSART_RXROUTE_PIN_SHIFT) );
   MX25_EUSART_ROUTE.TXROUTE   = ( (MX25_PORT_MOSI << _GPIO_EUSART_TXROUTE_PORT_SHIFT)
                                  | (MX25_PIN_MOSI  << _GPIO_EUSART_TXROUTE_PIN_SHIFT) );
   MX25_EUSART_ROUTE.ROUTEEN   = (  GPIO_EUSART_ROUTEEN_RXPEN
...
#elif defined(MX25_EUSART)
  EUSART_Reset(MX25_EUSART);
  MX25_EUSART_ROUTE.SCLKROUTE = _GPIO_EUSART_SCLKROUTE_RESETVALUE;
  MX25_EUSART_ROUTE.RXROUTE  = _GPIO_EUSART_RXROUTE_RESETVALUE;
  MX25_EUSART_ROUTE.TXROUTE  = _GPIO_EUSART_TXROUTE_RESETVALUE;
  MX25_EUSART_ROUTE.ROUTEEN  = _GPIO_EUSART_ROUTEEN_RESETVALUE;
  CMU_ClockEnable( MX25_EUSART_SCLK, false );
#endif
...
#elif defined(MX25_EUSART)
     EUSART_Spi_TxRx(MX25_EUSART, 0xff);
#endif
   }
}
...
#elif defined(MX25_EUSART)
      EUSART_Spi_TxRx(MX25_EUSART, byte_value);
#endif
      break;
#endif
#ifdef DIO
...
#elif defined(MX25_EUSART)
      data_buf = EUSART_Spi_TxRx(MX25_EUSART, 0xff);
#endif
      break;
#endif
#ifdef DIO
