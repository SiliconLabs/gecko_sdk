...
zaf_event_distributor_app_event_manager(const uint8_t event)
{
  ...
  switch (event) {
    case EVENT_APP_BUTTON_INCLUDE_DEVICE:
      ...
      break;
    case EVENT_APP_BUTTON_EXCLUDE_DEVICE:
      if (zaf_event_distributor_is_primary_controller()
      ...
      break;
    case EVENT_APP_BUTTON_ASSOCIATION_GROUP_ADD:
      ...
      break;
    case EVENT_APP_BUTTON_ASSOCIATION_GROUP_REMOVE:
      if (network_management_state == NETWORK_MANAGEMENT_STATE_REMOVE_GROUP
          || network_management_state == NETWORK_MANAGEMENT_STATE_ADD_GROUP) {
        terminateAssociation();
      } else {
    ...
    case EVENT_APP_NEXT_EVENT_JOB:
    {
      uint8_t next_event;
      /*check job-queue*/
      if (true == ZAF_JobHelperJobDequeue(&next_event)) {
        /*Let the event scheduler fire the event on state event machine*/
        ZAF_EventHelperEventEnqueue(next_event);
      } else {
    ...
    case EVENT_APP_BUTTON_BASIC_ON:
    ...
    case EVENT_APP_BUTTON_BASIC_OFF:
      //Since Association and Basic On/OFF shares buttons in this example application
      //the Basic On/Off events are allowed to terminate Association.
      if (network_management_state == NETWORK_MANAGEMENT_STATE_REMOVE_GROUP
          || network_management_state == NETWORK_MANAGEMENT_STATE_ADD_GROUP) {
      ...
      break;
    case EVENT_APP_SEND_BASIC_ON_JOB:
      ...
      break;
    case EVENT_APP_SEND_BASIC_OFF_JOB:
      ...
      break;
    case EVENT_APP_FINISH_EVENT_JOB:
      if (network_management_state == NETWORK_MANAGEMENT_STATE_REMOVE_GROUP
          || network_management_state == NETWORK_MANAGEMENT_STATE_ADD_GROUP) {
        terminateAssociation();
      } else {
      ...
      break;
    case EVENT_APP_BUTTON_NETWORK_LEARNMODE_NWI:
      ...
      break;
    case EVENT_APP_BUTTON_NETWORK_LEARNMODE_NWE:
      if (!zaf_event_distributor_is_primary_controller()) {
        if (SetRFReceiveMode(true)) {
          DPRINT("\nSet LearnMode NWE\n");
