...
static void SendEventToApp(uint8_t event, bool is_called_from_isr)
{
  ...
  {
    ZAF_EventHelperEventEnqueueFromISR(event);
  }
  else
  {
...
static void ButtonTimerCallback(SSwTimer *pTimer)
{
  ...
  {
    TimerStopFromISR(&m_button_timer);
...
/**
 * Configures a button based on configuration data definitions
 *
 * The Button GPIO port and pin configuration as defined in
 * extension_board_xxx.h or radio_board_xxx.h are used implicitly.
 *
 * @param btn  Button identifier.
 */
static void Board_ConfigButton(button_id_t btn)
{
  if (Board_IsButtonAvailable(btn))
  {
...
#endif
      AppTimerRegister(&m_button_timer, true, ButtonTimerCallback);
      GPIOINT_Init();
...
// UART init is handled by iostream_recommended_stream automatically
  ...
  //GPIO_PinModeSet(UART0_TX_PORT, UART0_TX_PIN, gpioModeInputPullLow, 0);
#else
  ZAF_UART0_init(UART0_TX_PORT, UART0_TX_PIN, UART0_RX_PORT, UART0_RX_PIN);
#endif

#ifdef DISABLE_USART1
  //GPIO_PinModeSet(UART1_TX_PORT, UART1_TX_PIN, gpioModeInput, 0);
#else
