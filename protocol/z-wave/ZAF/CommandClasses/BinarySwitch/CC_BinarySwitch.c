...
static void CC_BinarySwitch_report_stx(
  ...
  uint8_t * pFrameOutLength)
{
  ...
  uint8_t endpoint = rxOpt->destNode.endpoint;
  ...
  switch (pCmd->ZW_Common.cmd)
  {
    ...
    case SWITCH_BINARY_GET:
      ...
      pFrameOut->ZW_SwitchBinaryReportV2Frame.cmdClass     = COMMAND_CLASS_SWITCH_BINARY_V2;
      pFrameOut->ZW_SwitchBinaryReportV2Frame.cmd          = SWITCH_BINARY_REPORT_V2;
       ...
       */
      pFrameOut->ZW_SwitchBinaryReportV2Frame.currentValue = p_switch->old_value;
      ...
      pFrameOut->ZW_SwitchBinaryReportV2Frame.targetValue  = target_value;
      pFrameOut->ZW_SwitchBinaryReportV2Frame.duration     = ZAF_Actuator_GetDurationRemaining(&p_switch->actuator);

      *pFrameOutLength = sizeof(ZW_SWITCH_BINARY_REPORT_V2_FRAME);
      break;
    ...
    case SWITCH_BINARY_SET:
    {
      ...
      {
        pCmd->ZW_SwitchBinarySetV2Frame.duration = p_switch->default_duration;
      }

      if (0 == pCmd->ZW_SwitchBinarySetV2Frame.duration) {
        /*
    ...
    void * p_switch)
{
  ...
  pTxBuf->ZW_SwitchBinaryReportV2Frame.cmdClass     = COMMAND_CLASS_SWITCH_BINARY;
  pTxBuf->ZW_SwitchBinaryReportV2Frame.cmd          = SWITCH_BINARY_REPORT;
  pTxBuf->ZW_SwitchBinaryReportV2Frame.currentValue = p_switch_data->old_value;
  uint8_t target_value = ZAF_Actuator_GetTargetValue(&p_switch_data->actuator);
  // Encode target value in accordance with CC:0025.02.03.11.004.
  target_value = encode_value(target_value);
  pTxBuf->ZW_SwitchBinaryReportV2Frame.targetValue  = target_value;
  pTxBuf->ZW_SwitchBinaryReportV2Frame.duration     = ZAF_Actuator_GetDurationRemaining(&p_switch_data->actuator);
 ...
 */
REGISTER_CC_V4(COMMAND_CLASS_SWITCH_BINARY, SWITCH_BINARY_VERSION_V2, CC_BinarySwitch_handler, basic_set_mapper, basic_get_mapper, lifeline_reporting, 0, init_and_reset, init_and_reset);
