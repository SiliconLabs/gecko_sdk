...
ZW_WEAK const AGI_TABLE* cc_agi_get_config(void)
{
                 ...
                 uint8_t endpoint)
{
  ...
  if ((IS_NULL(pGroupName)) || (ZAF_CONFIG_NUMBER_OF_END_POINTS < endpoint))
  {
    return nameLength;
  }
  
  const AGI_TABLE* myAgi = cc_agi_get_config();
  ...
  else
  {
    ...
    else
    {
       ...
       */
      pSourceGroupName =(char*) (myAgi->tableEndpoint[endpoint].pTable[groupId - 2].groupName);
    }
  }
                             ...
                             AGI_TABLE_EP* pAgiTable)
{
  ...
  for (grpId = 0; grpId < pAgiTable->tableSize; grpId++)
  {
        ...
        (profile.profile_LS == pAgiTable->pTable[grpId].profile.profile_LS))
    {
      ...
      {
        if ( true == SearchCmdClass(cmdGrp, (cc_group_t*)&pAgiTable->pTable[grpId].cmdGrp[cmdGrpId], 1))
        {
                                       ...
                                       const uint8_t sourceEndpoint)
{
  ...
  grpId = SearchProfile(*pProfile, *pCurrentCmdGrp, (AGI_TABLE_EP*)&myAgi->tableEndpoint[sourceEndpoint]);
