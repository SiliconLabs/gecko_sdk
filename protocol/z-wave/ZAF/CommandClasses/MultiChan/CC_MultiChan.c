  ...
  switch(pCmd->ZW_Common.cmd)
  {
    ...
    case MULTI_CHANNEL_CAPABILITY_GET_V4:
      ...
      if ((0 == (pCmd->ZW_MultiChannelCapabilityGetV4Frame.properties1 & 0x7F)) ||
          (ZAF_CONFIG_NUMBER_OF_END_POINTS < (pCmd->ZW_MultiChannelCapabilityGetV4Frame.properties1 & 0x7F)))
      {
        return RECEIVED_FRAME_STATUS_FAIL;
      }

      {
        ...
        if (IS_NULL(pCmdClassList) || IS_NULL(pCmdClassList->cc_list) || (0 == pCmdClassList->list_size) || IS_NULL(pDevClass) || (0 == (pCmdCap->properties1 & 0x7f)))
        {
          return RECEIVED_FRAME_STATUS_FAIL;
        }
        else
        {
          ...
          memcpy( &(pTxBuf->ZW_MultiChannelCapabilityReport4byteV4Frame.commandClass1), pCmdClassList->cc_list, pCmdClassList->list_size);
          ccListSize = pCmdClassList->list_size;
        }
