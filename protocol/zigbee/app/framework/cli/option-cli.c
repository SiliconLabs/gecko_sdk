...
void emAfCliServiceDiscoveryCallback(const EmberAfServiceDiscoveryResult* result)
{
  ...
  || (defined(UC_BUILD) && defined(SL_CATALOG_ZIGBEE_DEBUG_PRINT_PRESENT)))
...
#endif //((defined(EMBER_AF_PRINT_ENABLE) && defined(EMBER_AF_PRINT_APP)) || (defined(UC_BUILD) && defined(SL_CATALOG_ZIGBEE_DEBUG_PRINT_PRESENT)))
}
