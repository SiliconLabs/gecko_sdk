...
uint32_t emberAfKeyEstablishmentClusterServerCommandParse(sl_service_opcode_t opcode,
    ...
    switch (cmd->commandId) {
      ...
      case ZCL_TERMINATE_KEY_ESTABLISHMENT_COMMAND_ID:
      {
        ...
        if (cmd->bufLen < payloadOffset + TERMINATE_KEY_ESTABLISHMENT_COMMAND_MIN_LENGTH) {
          ...
          if (zclStatus == EMBER_ZCL_STATUS_SUCCESS) {
            emberAfKeyEstablishmentClusterTerminateKeyEstablishmentCallback(cmd_data.statusCode,
                                                                            cmd_data.waitTime,
...
uint32_t emberAfKeyEstablishmentClusterClientCommandParse(sl_service_opcode_t opcode,
                                                          sl_service_function_context_t *context)
{
  (void)opcode;
