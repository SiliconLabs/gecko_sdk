...
#if (EMBER_AF_PLUGIN_POLL_CONTROL_SERVER_ACCEPT_SET_LONG_POLL_INTERVAL_COMMAND == 1)
...
bool emberAfPollControlClusterSetLongPollIntervalCallback(EmberAfClusterCommand *cmd)
{
...
bool emberAfPollControlClusterSetShortPollIntervalCallback(EmberAfClusterCommand *cmd)
{
                                                     ...
                                                     sl_service_function_context_t *context)
{
  ...
  if (!cmd->mfgSpecific) {
    switch (cmd->commandId) {
      ...
      {
        wasHandled = emberAfPollControlClusterCheckInResponseCallback(cmd);
        break;
      }
      ...
      {
        wasHandled = emberAfPollControlClusterSetLongPollIntervalCallback(cmd);
        break;
      }
      case ZCL_SET_SHORT_POLL_INTERVAL_COMMAND_ID:
      {
        wasHandled = emberAfPollControlClusterSetShortPollIntervalCallback(cmd);
        break;
      }
    }
  }
