...
bool emberAfPrepaymentClusterConsumerTopUpCallback(EmberAfClusterCommand *cmd)
{
  ...
  if ( !emberAfPluginPrepaymentServerConsumerTopUpCallback(cmd_data.originatingDevice, cmd_data.topUpCode) ) {
...
bool emberAfPrepaymentClusterGetTopUpLogCallback(EmberAfClusterCommand *cmd)
{
  ...
  emberAfPrepaymentClusterPrintln("RX: GetTopUpLog, endTime=0x%4x, numRec=%d", cmd_data.latestEndTime, cmd_data.numberOfRecords);
  ...
  emberAfPluginSendPublishTopUpLog(nodeId, srcEndpoint, dstEndpoint, cmd_data.latestEndTime, cmd_data.numberOfRecords);
