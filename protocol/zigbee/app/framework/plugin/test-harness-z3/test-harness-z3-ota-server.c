 ...
 */
WEAK(uint32_t emberAfOtaStorageGetTotalImageSizeCallback(const EmberAfOtaImageId* id))
{
  if (id != NULL) {
    if (id->manufacturerId == loadedOtaImageId.manufacturerId
        && id->imageTypeId == loadedOtaImageId.imageTypeId) {
...
WEAK(bool emberAfIsOtaImageIdValid(const EmberAfOtaImageId* idToCompare))
{
  EmberAfOtaImageId InvalidId = {
    INVALID_MANUFACTURER_ID,
    INVALID_DEVICE_ID,
...
WEAK(uint8_t emberAfOtaServerQueryCallback(const EmberAfOtaImageId* currentImageId,
