                                                ...
                                                uint16_t payloadLength)
{
  if (payloadBuffer != NULL
    ...
    emberAfSubGhzClusterPrintln("%s: Mgmt_NWK_Unsolicited_Enhanced_Update_notify received",
                                PLUGIN_NAME_STR);
...
# ifdef SL_CATALOG_ZIGBEE_OTA_SERVER_PRESENT
        if (dcState >= EMBER_DUTY_CYCLE_LBT_CRITICAL_THRESHOLD_REACHED
          ...
          emberAfCorePrintln("%s: client %2x %s within its suspend period.",
                             PLUGIN_NAME_STR,
                             sender,
                                  ...
                                  EmberPerDeviceDutyCycle *arrayOfDutyCycles)
{
  ...
  emberAfSubGhzClusterPrintln("%s: Duty Cycle handler called: page=%d, chan=%d, state=%d (previous=%d)",
                              PLUGIN_NAME_STR,
                              channelPage,
                              channel,
                              newState,
  ...
  if (emberAfPluginSubGhzDutyCycleCallback(channelPage,
    ...
    for (i = 1; i < totalDevices; ++i) {
      ...
      emberAfSubGhzClusterPrintln("%s: Checking client %2x, %s = %d",
                                  PLUGIN_NAME_STR,
                                  currentNode,
                                  "duty cycle",
                                  currentDutyCycle);
      ...
      if (currentNode != 0xFFFF                 // skip unused entries
        ...
        if (!client
          ...
          emberAfSubGhzClusterPrintln("%s: Found new candidate to suspend",
                                      PLUGIN_NAME_STR);
        }
      }
    }
    ...
    if (chosenNodeId != EMBER_NULL_NODE_ID) {
      emberAfSubGhzClusterPrintln("%s: Suspending client %2x",
                                  PLUGIN_NAME_STR,
                                  chosenNodeId);
