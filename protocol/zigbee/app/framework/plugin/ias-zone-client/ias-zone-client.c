...
bool emberAfIasZoneClusterZoneEnrollRequestCallback(EmberAfClusterCommand *cmd)
{
                                                 ...
                                                 sl_service_function_context_t *context)
{
  ...
  if (!cmd->mfgSpecific) {
    switch (cmd->commandId) {
      ...
      {
        wasHandled = emberAfIasZoneClusterZoneStatusChangeNotificationCallback(cmd);
        break;
      }
      case ZCL_ZONE_ENROLL_REQUEST_COMMAND_ID:
      {
        wasHandled = emberAfIasZoneClusterZoneEnrollRequestCallback(cmd);
        break;
      }
    }
  }
