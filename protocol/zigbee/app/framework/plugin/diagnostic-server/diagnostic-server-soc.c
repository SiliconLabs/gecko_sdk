...
bool emberAfPreMessageReceivedCallback(EmberAfIncomingMessage* incomingMessage)
{
  ...
  {
    uint8_t lqi;
    EmberStatus status = emberGetLastHopLqi(&lqi);
  ...
  {
    int8_t rssi;
    EmberStatus status = emberGetLastHopRssi(&rssi);
    if (EMBER_SUCCESS == status) {
      afStatus = emberAfWriteServerAttribute(
        endpoint,
        ZCL_DIAGNOSTICS_CLUSTER_ID,
...
#if defined (ZCL_USING_DIAGNOSTICS_CLUSTER_AVERAGE_MAC_RETRY_PER_APS_MSG_SENT_ATTRIBUTE)
  {
    ...
    if (EMBER_ZCL_STATUS_SUCCESS != afStatus) {
      emberAfCorePrintln("Diagnostic Server (SOC): failed to write value 0x%2x "
                         " to cluster 0x%2x attribute ID 0x%2x: error 0x%x",
                         macRetriesPerAps,
                         ZCL_DIAGNOSTICS_CLUSTER_ID,
                         ZCL_AVERAGE_MAC_RETRY_PER_APS_MSG_SENT_ATTRIBUTE_ID,
