...
bool emberAfOnOffClusterOffWithEffectCallback(EmberAfClusterCommand *cmd)
{
  ...
  if (cmd_data.effectId > EMBER_ZCL_ON_OFF_EFFECT_IDENTIFIER_DYING_LIGHT
      || cmd_data.effectVariant > EMBER_ZCL_ON_OFF_DELAYED_ALL_OFF_EFFECT_VARIANT_50_PERCENT_DIM_DOWN_IN_0P8_SECONDS_THEN_FADE_TO_OFF_IN_12_SECONDS
      || (cmd_data.effectId == EMBER_ZCL_ON_OFF_EFFECT_IDENTIFIER_DYING_LIGHT
          && cmd_data.effectVariant > EMBER_ZCL_ON_OFF_DYING_LIGHT_EFFECT_VARIANT_20_PERCENTER_DIM_UP_IN_0P5_SECONDS_THEN_FADE_TO_OFF_IN_1_SECOND)) {
    goto kickout;
  }
...
bool emberAfOnOffClusterOnWithTimedOffCallback(EmberAfClusterCommand *cmd)
{
                                                  ...
                                                  sl_service_function_context_t *context)
{
  ...
  if (!cmd->mfgSpecific) {
    switch (cmd->commandId) {
      ...
      {
        wasHandled = emberAfOnOffClusterOffWithEffectCallback(cmd);
        break;
      }
      ...
      {
        wasHandled = emberAfOnOffClusterOnWithTimedOffCallback(cmd);
        break;
      }
    }
  }
