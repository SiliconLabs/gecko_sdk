...
bool emberAfEventsClusterGetEventLogCallback(EmberAfClusterCommand *cmd)
{
...
bool emberAfEventsClusterClearEventLogRequestCallback(EmberAfClusterCommand *cmd)
{
  ...
  emberAfEventsClusterPrintln("RX: ClearEventLogRequest 0x%X", cmd_data.logId);
  ...
  if ((endpoint == EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT)
      && (cmd_data.logId == EMBER_ZCL_EVENT_LOG_ID_SECURITY_EVENT_LOG)) {
  ...
  if (emberAfPluginEventsServerOkToClearLogCallback((EmberAfEventLogId)cmd_data.logId)) {
    if (!emberAfEventsServerClearEventLog(endpoint, (EmberAfEventLogId)cmd_data.logId)) {
      emberAfEventsClusterPrintln("%p%p%p",
                                  "Error: ",
  ...
  } else if (EMBER_ZCL_EVENT_LOG_ID_ALL_LOGS == cmd_data.logId) {
                                                ...
                                                sl_service_function_context_t *context)
{
  ...
  if (!cmd->mfgSpecific) {
    switch (cmd->commandId) {
      ...
      {
        wasHandled = emberAfEventsClusterGetEventLogCallback(cmd);
        break;
      }
      case ZCL_CLEAR_EVENT_LOG_REQUEST_COMMAND_ID:
      {
        wasHandled = emberAfEventsClusterClearEventLogRequestCallback(cmd);
        break;
      }
    }
  }
