...
void emberAfPluginSimpleMeteringServerSamplingEventHandler(sl_zigbee_event_t * event);
void emberAfPluginSimpleMeteringServerSupplyEventHandler(sl_zigbee_event_t * event);
                                                        ...
                                                        uint8_t endpoint)
{
  ...
  sl_zigbee_event_set_delay_ms(samplingEvent,
                               (delay * 1000));
  return sampleId;
}
...
void emberAfPluginSimpleMeteringServerSamplingEventHandler(sl_zigbee_event_t * event)
{
  ...
  EmberStatus status;
  sl_zigbee_event_set_inactive(samplingEvent);
...
void emberAfPluginSimpleMeteringServerSupplyEventHandler(sl_zigbee_event_t * event)
{
  uint32_t changeTime;
  uint32_t currentTime;
  sl_zigbee_event_set_inactive(supplyEvent);
  ...
  if (changeTime > currentTime) {
    ...
    sl_zigbee_event_set_delay_ms(supplyEvent,
                                 delay * 1000);
  } else {
