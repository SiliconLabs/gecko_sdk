...
EzspStatus sli_zigbee_secure_ezsp_encode(void)
{
  ...
  #else
  uint8_t packetLength = SECURE_EZSP_AUTHENTICATE_SIZE + EZSP_EXTENDED_FRAME_ID_LENGTH + parametersLength;
...
EzspStatus sli_zigbee_secure_ezsp_decode(void)
{
  ...
  #else
  uint8_t packetLength = SECURE_EZSP_AUTHENTICATE_SIZE + EZSP_EXTENDED_FRAME_ID_LENGTH + parametersLength;
  if (sl_zb_sec_man_aes_ccm(nonce,
                            false,
                            ezspFrameContents,
                            SECURE_EZSP_AUTHENTICATE_SIZE,
                            packetLength,
                                                 ...
                                                 SecureEzspRandomNumber *ncpRandomNumber)
{
  ...
  sl_zb_sec_man_import_key(&context, (sl_zb_sec_man_key_t *)&securityKey);
  sl_zb_sec_man_load_key_context(&context);
  sl_zb_sec_man_hmac_aes_mmo(combinedRandomNumbers,
                             SECURE_EZSP_RANDOM_NUMBER_SIZE * 2,
                             combinedSessionIds);
  #endif
...
bool sli_zigbee_secure_ezsp_is_security_key_set(void)
{
  ...
  #else
  sl_zb_sec_man_key_t ezspSecurityKey = { 0 };
  sl_zb_sec_man_context_t context;
  sl_zb_sec_man_init_context(&context);
  context.core_key_type = SL_ZB_SEC_MAN_KEY_TYPE_SECURE_EZSP_KEY;
  sl_zb_sec_man_export_key(&context, &ezspSecurityKey);
  // Token is all 0x00s when not set
  if (!byteCompare(ezspSecurityKey.key, EMBER_ENCRYPTION_KEY_SIZE, 0x00)) {
    arrayPrint("security key", ezspSecurityKey.key, EMBER_ENCRYPTION_KEY_SIZE);
    return true;
  }
  #endif
