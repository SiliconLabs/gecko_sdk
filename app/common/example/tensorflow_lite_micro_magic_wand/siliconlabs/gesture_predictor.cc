...
int previous_predict_index = 3; //No Gesture 
}  // namespace
...
int PredictGesture(float* output) {
  ...
  if ((max_predict_index == kNoGesture) ||
      ...
      ((max_predict_index == previous_predict_index) && (prediction_suppression_count > 0))) {
    return kNoGesture;
  } else {
    ...
    previous_predict_index = max_predict_index;
