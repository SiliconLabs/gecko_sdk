...
typedef enum sn_coap_status_ {
  ...
  COAP_STATUS_PARSER_DUPLICATED_MSG          = 2,   /**< CoAP will send Acknowledgment message to duplicated message sender */
  ...
  COAP_STATUS_PARSER_BLOCKWISE_ACK           = 4,   /**< Acknowledgment for sent Blockwise message received */
 ...
 *
 * \brief Parse CoAP message from given Packet data.
 *
 ...
 *
 * \brief Release memory of given CoAP message.
 *
 ...
 *
 * \brief Build an outgoing message buffer from a CoAP header structure.
 *
 ...
 *
 * \brief Calculate needed Packet data memory size for given CoAP message
 *
 ...
 *
 * \brief Prepare a generic response packet from a request packet. This function allocates memory for the resulting sn_coap_hdr_s.
 *
 * \param *handle Pointer to CoAP library handle
...
/**
 * \brief Initialize a message structure to empty.
 *
 * \param *coap_msg_ptr is pointer to CoAP message to initialize
 *
...
/**
 * \brief Allocate and initialize options list structure.
 *
 * \param *handle Pointer to CoAP library handle
