 ...
 *****************************************************************************/
static bool _collector_send_request(sl_wisun_meter_entry_t *meter, sl_wisun_meter_request_t *req);
...
void sl_wisun_collector_measurement_request_loop(void)
{
  ...
  for (uint16_t i = 0; i < SL_WISUN_COLLECTOR_MAX_REG_METER; ++i) {
    ...
    res = _collector_hnd.send(&_meters[i], &_measurement_req);
    if (!res) {
...
static void _collector_recv_thread_fnc(void * args)
{
  ...
  res = bind(sockid, (const struct sockaddr *) &collector_addr,
  ...
  SL_WISUN_THREAD_LOOP {
    ...
    if (_collector_hnd.recv != NULL) {
      meter = _collector_hnd.recv(sockid);
    }
    if (meter != NULL) {
