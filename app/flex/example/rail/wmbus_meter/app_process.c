...
void app_process_action(RAIL_Handle_t rail_handle)
{
  ...
  switch (state) {
    ...
    case S_TX_DONE:
      ...
      if ( get_wmbus_accessibility() == WMBUS_ACCESSIBILITY_LIMITED_ACCESS ) {
...
void sl_rail_util_on_event(RAIL_Handle_t rail_handle, RAIL_Events_t events)
{
  if ( events & RAIL_EVENTS_TX_COMPLETION ) {
    if ( events & RAIL_EVENT_TX_PACKET_SENT ) {
      ...
      last_tx_start_time = last_tx_end_time;
      RAIL_GetTxTimePreambleStart(rail_handle, last_tx_length, &last_tx_start_time);
