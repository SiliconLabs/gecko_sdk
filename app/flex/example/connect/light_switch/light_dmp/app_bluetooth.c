...
//                              Macros and Typedefs
// -----------------------------------------------------------------------------
#define DEVNAME_LEN                              (9) // length of the "LIGHT_DMP"
...
static response_data_t response_data = {
  .flags_len = 2,                    // length (incl type)
  .flags_type = 0x01,
  .flags = 0x04 | 0x02,              //Flags: LE General Discoverable Mode, BR/EDR is disabled
  .short_name_len = DEVNAME_LEN + 1,
  .short_name_type = 0x08,
  .short_name = "DM00:00",
  .uuid_length = UUID_LEN + 1,       // length of UUID data (incl type)
  .uuid_type =   0x06,               // incomplete list of service UUID's
  // custom service UUID for silabs light in little-endian format
  .uuid = { 0x13, 0x87, 0x37, 0x25, 0x42, 0xb0, 0xc3, 0xbf, 0x78, 0x40, 0x83, 0xb5, 0xe4, 0x96, 0xf5, 0x63 }
};

/// name of the application
static char adv_name[] = "LIGHT";
...
sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      memcpy((uint8_t*) &response_data.short_name, adv_name, sizeof(adv_name));
      ...
      sl_bt_advertiser_set_data(advertising_set_handle, 0,
      ...
      bt_status = sl_bt_advertiser_start(advertising_set_handle,
    ...
    case sl_bt_evt_connection_closed_id:
      ...
      if (boot_to_dfu) {
        ...
        bt_status = sl_bt_advertiser_start(0,
                                           advertiser_general_discoverable,
                                           advertiser_connectable_scannable);
      }
      break;
          ...
          switch (indication->characteristic) {
            ...
            case gattdb_light_state_connect:
            {
              ...
              if (bt_status != SL_STATUS_OK) {
                app_log_error("sl_bt_gatt_server_send_notification failed with 0x%04X\n",
                              bt_status);
              }
