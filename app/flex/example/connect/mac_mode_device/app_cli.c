...
void cli_set_security_key(sl_cli_command_arg_t *arguments)
{
  EmberKeyData key;
  size_t hex_length = 0;
  uint8_t *hex_value = sl_cli_get_argument_hex(arguments, 0, &hex_length);

  if (hex_length != EMBER_ENCRYPTION_KEY_SIZE) {
    app_log_error("Security key set failed, not correct length\n");
    return;
  }

  memcpy(key.contents, hex_value, hex_length);
