...
class BtmeshConfCmd(BtmeshCmd):
    ...
    def create_conf_ae_parser(self, subparsers) -> Tuple[str, ArgumentParserExt]:
        ...
        self.conf_ae_parser: ArgumentParserExt = subparsers.add_parser(
            ...
            "Silabs BT Mesh SOC and NCP examples by default so some additional "
            "steps are necessary to add it to the embedded code. "
            "The NCP projects shall contain the NCP Advertisement Extension Client "
            "and NCP Advertisement Extension Server components. The SOC projects "
            "shall contain Advertisement Extension Server component. "
            "These components pull in the Silicon Labs Configuration Client Model "
            "and Silicon Labs Configuration Server Model components respectively. "
            "All AE components require the Proprietary Extended Advertiser and "
            "Standard Extended Scanner components so these are added to project "
            "automatically as well. "
            "WARNING! If Standard Extended Scanner component is added to the "
            "project then the BLE LL layer follows the AuxPtr in AE packets "
