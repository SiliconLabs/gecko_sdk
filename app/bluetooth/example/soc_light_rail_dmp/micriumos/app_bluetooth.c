...
typedef struct {
  ...
  uint8_t min_num[2];     // Beacon minor number. Used to specify
                          // individual beacons within a group.
  uint8_t tx_power;       // The measured RSSI at 1 m distance [dBm]. See the
                          // iBeacon specification for measurement guidelines.
} i_beacon_data_t;
...
typedef struct {
  ...
  uint8_t tx_power;           // The measured RSSI at 0 m distance [dBm]
...
static i_beacon_data_t i_beacon_data = {
  ...
  // Beacon type: 0x0215 is iBeacon
  { UINT16_TO_BYTE1(0x0215), UINT16_TO_BYTE0(0x0215) },

  // 128 bit / 16 byte UUID - generated specially for the DMP Demo
  { 0x00, 0x47, 0xe7, 0x0a, 0x5d, 0xc1, 0x47, 0x25,
    0x87, 0x99, 0x83, 0x05, 0x44, 0xae, 0x04, 0xf6 },
...
static eddystone_data_t eddystone_data = {
  // Flag bits - See Bluetooth 4.0 Core Specification , Volume 3,
  // Appendix C, 18.1 for more details on flags.
  2,                // length
  0x01,             // type
  0x04 | 0x02,      // Flags: LE General Discoverable Mode, BR/EDR is disabled.
  0x03,             // Service field length
  0x03,             // Service field type
  { UINT16_TO_BYTES(0xFEAA) }, // 16-bit Eddystone UUID
  0x10, // Eddystone-TLM Frame length
  0x16, // Service Data data type value
  { UINT16_TO_BYTES(0xFEAA) }, // 16-bit Eddystone UUID
  0x10, // Eddystone-URL Frame type
  0x01, // Tx power
  0x00, // URL prefix - standard
  { 's', 'i', 'l', 'a', 'b', 's', '.', 'c', 'o', 'm' } // URL
};
...
/**************************************************************************//**
 * Enabling Beacon advertistments
 *
 *****************************************************************************/
static void beacon_advertisements(void)
{
  // Advertistment data parameters
  ...
  sl_bt_advertiser_set_data(advertising_set_handle1,
                            0,
                            adv_data_len,
                            (uint8_t *)&adv_data);
  ...
  sc = sl_bt_advertiser_start(advertising_set_handle1,
                              sl_bt_advertiser_user_data, // discoverable mode
                              sl_bt_advertiser_non_connectable); // connectable mode
  app_assert_status(sc);
  ...
  // Set the response data
  sl_bt_advertiser_set_data(advertising_set_handle2,
                            0, adv_data_len,
                            (uint8_t *)&adv_data);
