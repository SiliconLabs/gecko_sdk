...
/***************************************************************************//**
* Helper script to print a given Bluetooth address
*
* @param[in] uuid Pointer to the Bluetooth address
*******************************************************************************/
static void print_mac(bd_addr *mac);
    ...
    case UI_START:
      if (WAITING_FOR_INPUT != ui_state) {
        ...
        if (true == input_ready) {
          ...
          } else if (MAC_LEN_WITHOUT_SEPARATORS == len
                     || MAC_LEN_WITH_SEPARATORS == len) {
            bd_addr addr;
            app_parse_mac(input_buffer, len, &addr);
            sc = btmesh_prov_start_provisioning_by_mac(netkey_index, addr, 0);
...
{
  app_log("MAC:     %02x:%02x:%02x:%02x:%02x:%02x" APP_LOG_NEW_LINE,
          mac->addr[5],
          mac->addr[4],
          mac->addr[3],
          mac->addr[2],
