...
Components required:
1. btmesh_provisioner host example
Example host application for demonstrating provisioner capabilities.

2. ncp_btmesh_empty target sample app
...
1. NCP target
    – ncp_btmesh_empty Target sample app must be programmed to the EFR32 chip:
...
2. btmesh_provisioner
    - Open a new terminal (MSYS2 or MinGW64 terminal on Windows)
    - Navigate to app/bluetooth/example_host/empty_btmesh in Bt Mesh SDK
    - Build the sample app by typing 'make'
    - Run the sample app with appropriate parameters (e.g. ./exe/btmesh_provisioner.exe -u /dev/ttyS4 --scan)
...
1. UI mode
    ...
    e.g. btmesh_provisioner.exe -u COM5.
    ...
    just run the program as e.g. 'winpty exe/btmesh_provisioner.exe -u COM5'
