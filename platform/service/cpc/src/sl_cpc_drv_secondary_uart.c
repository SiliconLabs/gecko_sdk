...
#define SL_CPC_DRV_UART_RX_MAX_BUFFER_SIZE (SL_MIN(((uint16_t)(SLI_CPC_RX_DATA_MAX_LENGTH + SLI_CPC_HDLC_HEADER_RAW_SIZE)), DMA_MAX_XFER_LEN))
...
sl_status_t sli_cpc_drv_init(void)
{
  ...
  rx_descriptor[0] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_P2M_BYTE(&(SL_CPC_DRV_UART_PERIPHERAL->RXDATA), buffer_ptr, SL_MIN((uint16_t)(SLI_CPC_RX_DATA_MAX_LENGTH + SLI_CPC_HDLC_HEADER_RAW_SIZE), DMA_MAX_XFER_LEN));

  if (sli_cpc_get_raw_rx_buffer(&buffer_ptr) != SL_STATUS_OK) {
    EFM_ASSERT(false);
    return SL_STATUS_ALLOCATION_FAILED;
  }
  rx_descriptor[1] = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_SINGLE_P2M_BYTE(&(SL_CPC_DRV_UART_PERIPHERAL->RXDATA), buffer_ptr, SL_MIN((uint16_t)(SLI_CPC_RX_DATA_MAX_LENGTH + SLI_CPC_HDLC_HEADER_RAW_SIZE), DMA_MAX_XFER_LEN));
...
static void restart_dma(void)
{
  ...
  // Restore xferCnt
  rx_descriptor_head->xfer.xferCnt =  SL_MIN((uint16_t)(SLI_CPC_RX_DATA_MAX_LENGTH + SLI_CPC_HDLC_HEADER_RAW_SIZE), DMA_MAX_XFER_LEN) - 1;
