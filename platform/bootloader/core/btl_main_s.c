...
#endif
#if defined(TEST_BOOTLOADER_NS)
extern void prepare_bootloader_ns_test(void);
#endif
...
/**************************************************************************//**
 * @brief Clean up the FRCRAM that contain root key before
 *        jumping in to the Non-secure bootloader
 *****************************************************************************/
static inline void cleanUpFRCRAM(void)
{
  ...
  volatile uint32_t *volatile srk
    = (volatile uint32_t *volatile)(vse_reply + VSE_REPLY_SIZE_WORDS + vse_data_length + 1);

  // Clear the SRK from the mailbox location
  for (size_t i = 0u; i < sizeof(vse_srk_t) / sizeof(uint32_t); i++) {
    *(srk + i) = 0u;
  }
}
...
#endif
#if defined(CRYPTOACC_PRESENT)
  // Clean up the FRCRAM region to prevent the NS bootloader from extracting the root key
  cleanUpFRCRAM();
#endif
#if defined(BOOTLOADER_APPLOADER)
