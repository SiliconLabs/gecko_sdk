...
RAIL_Status_t disableIncompatibleProtocols(RAIL_PtiProtocol_t newProtocol)
{
  ...
  if ((newProtocol != RAIL_PTI_PROTOCOL_802154)
      && RAIL_IEEE802154_IsEnabled(railHandle)) {
    status = RAIL_IEEE802154_Deinit(railHandle);
  ...
  if ((newProtocol != RAIL_PTI_PROTOCOL_ZWAVE)
    ...
    status = RAIL_ZWAVE_Deinit(railHandle);
    if (status != RAIL_STATUS_NO_ERROR) {
      return status;
    }
  }
  return RAIL_SetPtiProtocol(railHandle, newProtocol);
}
...
void deinitProtocol(sl_cli_command_arg_t *args)
{
  ...
  RAIL_Status_t status = disableIncompatibleProtocols(RAIL_PTI_PROTOCOL_CUSTOM);
