...
{
  CHECK_RAIL_HANDLE(sl_cli_get_command_string(args, 0));
  // Attempt to convert the power mode string to a RAIL_TxPowerMode_t enum index.
  RAIL_TxPowerMode_t mode = getPowerModeIndexFromString(sl_cli_get_argument_string(args, 0));
  if (mode == 0xFF) {
    // If the string doesn't match a powerMode, attempt to convert it to a RAIL_TxPowerMode_t.
    char *strtoulEnd;
    mode = (RAIL_TxPowerMode_t)strtoul(sl_cli_get_argument_string(args, 0), &strtoulEnd, 0);
    //check that strtoul didn't fail and that mode is valid RAIL_TxPowerMode_t
    if (strtoulEnd == sl_cli_get_argument_string(args, 0) || mode >= RAIL_TX_POWER_MODE_NONE) {
...
void setPower(sl_cli_command_arg_t *args)
{
  ...
  if (sl_cli_get_argument_count(args) >= 2 && strcmp(sl_cli_get_argument_string(args, 1), "raw") == 0) {
    ...
    if (RAIL_POWER_MODE_IS_ANY_DBM_POWERSETTING_MAPPING_TABLE(tempCfg.mode)) {
      // dBm-to-powerSetting mode does not support raw power setting
      responsePrint(sl_cli_get_command_string(args, 0), "%s does not support setting raw power.", powerModes[tempCfg.mode]);
      setPowerError = true;
    }
#endif
...
void sweepTxPower(sl_cli_command_arg_t *args)
{
  ...
  switch (txPowerConfig.mode) {
    ...
    case RAIL_TX_POWER_MODE_2P4GIG_LLP:
      start = RAIL_TX_POWER_LEVEL_2P4_LLP_MIN;
      end = RAIL_TX_POWER_LEVEL_2P4_LLP_MAX;
      break;
#endif
