 ...
 *
 * @note 802.15.4 radio configurations with Forward Error Correction (FEC)
 *   enabled are incompatible with 802.15.4 filtering and AutoACK on
 *   EFR32xG1 platforms.
 *   AutoACK should be disabled and promiscuous mode enabled when using such
 *   a configuration. This is enforced implicitly on EFR32xG1 platforms with
 *   \ref RAIL_IEEE802154_SUPPORTS_G_DYNFEC true when
 *   \ref RAIL_IEEE802154_ConfigGOptions() is called to enable any G options.
 */
...
typedef struct RAIL_IEEE802154_RxChannelSwitchingCfg {
   ...
   * This way, during configuration, the software can confirm it's
   * writing within the range of the buffer. The configuration API will return
   * an error if bufferBytes is insufficient.
   */
...
RAIL_ENUM(RAIL_IEEE802154_SignalIdentifierMode_t) {
  ...
  /* 2.4Ghz 802.15.4 signal detection mode with SI block reset on GPIO falling edge (via PRS). */
  RAIL_IEEE802154_SIGNAL_IDENTIFIER_MODE_154,
  /* 2.4Ghz 802.15.4 signal detection mode with SI block reset on AGC negative step. */
  RAIL_IEEE802154_SIGNAL_IDENTIFIER_MODE_154_WITH_AGC_RESET
};
...
#define RAIL_IEEE802154_SIGNAL_IDENTIFIER_MODE_154_WITH_AGC_RESET      ((RAIL_IEEE802154_SignalIdentifierMode_t)RAIL_IEEE802154_SIGNAL_IDENTIFIER_MODE_154_WITH_AGC_RESET)
#endif
 ...
 * peer and is only supported on certain Series-2 platforms.
 *
