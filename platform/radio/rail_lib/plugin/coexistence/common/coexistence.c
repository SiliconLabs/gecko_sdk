...
#if SL_RAIL_UTIL_COEX_OVERRIDE_GPIO_INPUT
  .index = COEX_GPIO_INDEX_INTERNAL_PWM_REQ,
#endif
...
__STATIC_INLINE void COEX_SetPriorityAndRequest(bool request, bool priority)
{
  ...
  sl_coex_wifi_sim_set_gpio_output(pwmReqHandle, true);
#else
  sl_coex_wifi_sim_set_gpio_output(reqHandle, request);
#endif
#endif
}
__STATIC_INLINE void COEX_ClearRequestAndPriority(void)
...
#ifdef SL_CATALOG_RAIL_UTIL_COEX_WIFI_SIMULATOR_PRESENT
#if SL_RAIL_UTIL_COEX_PWM_REQ_ENABLED && SL_RAIL_UTIL_COEX_REQ_SHARED
  sl_coex_wifi_sim_set_gpio_output(pwmReqHandle, false);
#else
  sl_coex_wifi_sim_set_gpio_output(reqHandle, false);
#endif
#endif
}
