...
class CALC_Synth_ocelot(CALC_Synth_lynx):
    ...
    def buildVariables(self, model):
        ...
        model.vars.synth_rx_mode.var_enum = CreateModelVariableEnum(
            ...
            member_data=[
                ...
                ['MODE2', 1, 'RX Mode 2'],
                ['MODE3', 2, 'RX Mode 3'],
                ['MODE4', 3, 'RX Mode 4'] #Targeted for 2.4GHz BLE
            ])
    ...
    def calc_synth_settling_mode(self, model):
        ...
        if rf_band == model.vars.rf_band.var_enum.BAND_2400:
            synth_settling_mode = model.vars.synth_settling_mode.var_enum.BLE
        else:
            #Set synth settling mode to Normal for now
            synth_settling_mode = model.vars.synth_settling_mode.var_enum.NORMAL

        model.vars.synth_settling_mode.value = synth_settling_mode
    ...
    def calc_rx_mode(self, model):
        ...
        if synth_settling_mode.value == model.vars.synth_settling_mode.var_enum.BLE:
            model.vars.synth_rx_mode.value = model.vars.synth_rx_mode.var_enum.MODE4
    ...
    def calc_rx_mode_reg(self, model):
        ...
        rx_mode_settings = {
            'SYNTH.LPFCTRL2RX.CASELRX':         [1,1,0,1],
            'SYNTH.LPFCTRL2RX.CAVALRX':         [8,16,0,16],
            'SYNTH.LPFCTRL2RX.CZSELRX':         [1,1,1,1],
            'SYNTH.LPFCTRL2RX.CZVALRX':         [70,128,254,229],
            'SYNTH.LPFCTRL2RX.CFBSELRX':        [0,0,0,0],
            'SYNTH.LPFCTRL2RX.LPFGNDSWENRX':    [0,0,0,0],
            'SYNTH.LPFCTRL2RX.MODESELRX':       [0,0,1,0],
            'SYNTH.LPFCTRL1RX.OP1BWRX':         [5,0,0,0],
            'SYNTH.LPFCTRL1RX.OP1COMPRX':       [13,7,10,7],
            'SYNTH.LPFCTRL1RX.RFBVALRX':        [0,0,2,0],
            'SYNTH.LPFCTRL1RX.RPVALRX':         [0,7,5,7],
            'SYNTH.LPFCTRL1RX.RZVALRX':         [3,14,9,13],
            'SYNTH.LPFCTRL2RX.LPFSWENRX':       [1,1,1,1],
            'SYNTH.DSMCTRLRX.REQORDERRX':       [0,0,0,0],
            'SYNTH.DSMCTRLRX.MASHORDERRX':      [0,1,0,1],
            'SYNTH.DSMCTRLRX.DEMMODERX':        [1,0,1,0],
            'SYNTH.DSMCTRLRX.LSBFORCERX':       [0,1,0,1],
            'SYNTH.DSMCTRLRX.DSMMODERX':        [0,1,0,1],
            'SYNTH.DSMCTRLRX.DITHERDACRX':      [3,0,3,0],
            'SYNTH.DSMCTRLRX.DITHERDSMOUTPUTRX':[7,0,7,0],
            'SYNTH.DSMCTRLRX.DITHERDSMINPUTRX': [1,0,1,0],
            'RAC.SYMMDCTRL.SYMMDMODERX':        [2,4,2,4],
            'RAC.SYTRIM1.SYLODIVLDOTRIMNDIORX': [4,4,4,4],
            'RAC.SYTRIM1.SYLODIVLDOTRIMCORERX': [0,0,0,0],
            'RAC.VCOCTRL.VCODETAMPLITUDERX':    [4,4,4,4],
            'RAC.SYTRIM0.SYCHPCURRRX':          [5,5,5,5],
            'RAC.SYTRIM0.SYCHPREPLICACURRADJ':  [1,1,1,1],
            'RAC.RX.SYCHPBIASTRIMBUFRX':        [1,0,1,0],
            'RAC.RX.SYPFDCHPLPENRX':            [0,1,0,1],
            'RAC.RX.SYPFDFPWENRX':              [0,1,0,1],
            'RAC.SYEN.SYCHPLPENRX':             [0,1,0,1],
            'RAC.SYTRIM0.SYCHPLEVPSRCRX':       [7,0,7,0],
            'RAC.SYTRIM0.SYCHPSRCENRX':         [1,0,1,0]}
    ...
    def calc_boostlna_reg(self, model):
        ...
        if rf_band == model.vars.rf_band.var_enum.BAND_2400:
            ...
            # https://jira.silabs.com/browse/MCUW_RADIO_CFG-1612 always set to 0 for SubG
            boostlna = 0
