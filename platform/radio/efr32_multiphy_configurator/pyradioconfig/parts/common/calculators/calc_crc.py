...
class CALC_CRC(ICalculator):
    ...
    def buildVariables(self, model):
        ...
        """
        #CRC POLY
        var = self._addModelVariable(model, 'crc_poly', Enum, ModelVariableFormat.DECIMAL, 'List of supported CRC Polynomials')
        member_data = [
            ['NONE' , 0, 'No CRC'],
            ['CRC_8',  1, 'X8+X2+X+1'],
        ...
        #CRC_PAD_INPUT
        self._addModelVariable(model, 'crc_pad_input', bool, ModelVariableFormat.ASCII, 'Set to true to enable zero padding of the CRC input data.')

        #CRC_INPUT_BIT_ORDER
        var = self._addModelVariable(model, 'crc_input_order', Enum, ModelVariableFormat.DECIMAL, 'Define the order data bits are fed into the CRC generator')
        member_data = [
        ...
        #Outputs
        """
        self._addModelRegister(model, 'CRC.CTRL.PADCRCINPUT'       , int, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'CRC.CTRL.BITSPERWORD'       , int, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'CRC.CTRL.BITREVERSE'       , int, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'CRC.CTRL.BYTEREVERSE'       , int, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'CRC.CTRL.INPUTBITORDER'       , int, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'CRC.CTRL.CRCWIDTH'       , int, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'CRC.CTRL.OUTPUTINV'       , int, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'CRC.INIT.INIT'       , long, ModelVariableFormat.HEX )
        self._addModelRegister(model, 'CRC.POLY.POLY'       , long, ModelVariableFormat.HEX )
    ...
    def _calc_init(self, model):
        ...
        """
        self._reg_write(model.vars.CRC_CTRL_PADCRCINPUT, 0)
        self._reg_write(model.vars.CRC_CTRL_BITREVERSE, 0)
        self._reg_write(model.vars.CRC_CTRL_BYTEREVERSE, 0)
        self._reg_write(model.vars.CRC_CTRL_INPUTBITORDER, 0)
        self._reg_write(model.vars.CRC_CTRL_CRCWIDTH, 0)
        self._reg_write(model.vars.CRC_CTRL_OUTPUTINV, 0)
        self._reg_write(model.vars.CRC_INIT_INIT, long(0))
        self._reg_write(model.vars.CRC_POLY_POLY, long(0))

        #Always set BITSPERWORD to 7 because we work with bytes
        self._reg_write(model.vars.CRC_CTRL_BITSPERWORD, 7)
    ...
    def calc_crc_settings(self, model):
        ...
        if model.vars.crc_poly.value.value != model.vars.crc_poly.var_enum.NONE.value:
          ...
          self._reg_write(model.vars.CRC_POLY_POLY, poly_reg )
          self._reg_write(model.vars.CRC_INIT_INIT, seed_reg)

          # Subtract 1 from the CRC size to get the proper value
          self._reg_write(model.vars.CRC_CTRL_CRCWIDTH, model.vars.crc_size.value - 1)

          #Handle Endianness
          if model.vars.crc_byte_endian.value == model.vars.crc_byte_endian.var_enum.LSB_FIRST:
            self._reg_write(model.vars.CRC_CTRL_BYTEREVERSE, 1)
          if model.vars.frame_bitendian.value.value == model.vars.crc_bit_endian.value.value:
            self._reg_write(model.vars.CRC_CTRL_BITREVERSE, 1)

          #Handle Input Bit Order
          if model.vars.crc_input_order.value == model.vars.crc_input_order.var_enum.MSB_FIRST:
            self._reg_write(model.vars.CRC_CTRL_INPUTBITORDER, 1)

          #Handle pad crc input
          if model.vars.crc_pad_input.value is True:
            self._reg_write(model.vars.CRC_CTRL_PADCRCINPUT, 1)

          if model.vars.crc_invert.value is True:
            self._reg_write(model.vars.CRC_CTRL_OUTPUTINV, 1)
