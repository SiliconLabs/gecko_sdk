 ...
 * Mbed TLS APIs. This is notably used on the NS side of TrustZone-enabled
 * applications.
 *
 * The NO_CRYPTO_ACCELERATION condition is kept around for backward
 * compatibility, but since it's not SL_ namespaced, we will only define
 * SL_MBEDTLS_NO_CRYPTO_ACCELERATION from SL components. */
#if !(defined(SL_MBEDTLS_NO_CRYPTO_ACCELERATION) || defined(NO_CRYPTO_ACCELERATION))
...
#endif /* !(SL_MBEDTLS_NO_CRYPTO_ACCELERATION || NO_CRYPTO_ACCELERATION) */
