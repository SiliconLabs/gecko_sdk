/***************************************************************************//**
 * @file
 * @brief MVP Math vector complex dot product functions.
 *******************************************************************************
 * # License
...
sl_status_t sl_math_mvp_complex_vector_dot_product_f16(float16_t *input_a,
                                                       float16_t *input_b,
                                                       size_t num_elements,
                                                       float16_t *output)
{
  ...
  len_matrix = (uint32_t)num_elements;
  len_remainder = 0;
  ...
  sli_mvp_pb_config_matrix(p->p, vector_z, (void*)output,  data_type, 1, 1, &status);
  ...
  if (len_remainder > 0) {
    sli_mvp_pb_config_vector(p->p, vector_x1, (void*)&input_a[ofs_remainder * 2], data_type, len_remainder, &status);
    sli_mvp_pb_config_vector(p->p, vector_y1, (void*)&input_b[ofs_remainder * 2], data_type, len_remainder, &status);
  ...
  MVP->INSTR[0].CFG2 = (SLI_MVP_OP(MACC) << _MVP_INSTRCFG2_ALUOP_SHIFT)
  ...
  if (len_remainder > 0) {
    ...
    MVP->ARRAY[4].ADDRCFG = (sli_mvp_addr_reg_t)&input_b[ofs_remainder * 2];
  ...
  MVP->INSTR[last_instruction_idx].CFG2 = (SLI_MVP_OP(COPY) << _MVP_INSTRCFG2_ALUOP_SHIFT)
