                                       ...
                                       size_t num_elements)
{
  ...
  if (len_remainder > 0) {
    ...
    // Program instructions.
    MVP->INSTR[2].CFG0 = SLI_MVP_ALUIN_A(SLI_MVP_R0, SLI_MVP_ALUIN_REALNEGATE | SLI_MVP_ALUIN_IMAGNEGATE)
                        | SLI_MVP_ALU_Z(SLI_MVP_R1);
    MVP->INSTR[2].CFG1 = SLI_MVP_LOAD(0, SLI_MVP_R0, SLI_MVP_ARRAY(2), SLI_MVP_INCRDIM_WIDTH);
    MVP->INSTR[2].CFG2 = (SLI_MVP_OP(COPY) << _MVP_INSTRCFG2_ALUOP_SHIFT)
                        | MVP_INSTRCFG2_LOOP2BEGIN;

    MVP->INSTR[3].CFG0 = SLI_MVP_ALU_A(SLI_MVP_R0)
                        | SLI_MVP_ALU_X(SLI_MVP_R1)
                        | SLI_MVP_ALU_Y(SLI_MVP_R1)
                        | SLI_MVP_ALU_Z(SLI_MVP_R3);
    MVP->INSTR[3].CFG1 = SLI_MVP_STORE(SLI_MVP_R3, SLI_MVP_ARRAY(3), SLI_MVP_INCRDIM_WIDTH);
    MVP->INSTR[3].CFG2 = (SLI_MVP_OP(MAXAC2) << _MVP_INSTRCFG2_ALUOP_SHIFT)
                        | MVP_INSTRCFG2_LOOP2END;

    // Move the next instruction index
    last_instruction_idx += 2;
  }
