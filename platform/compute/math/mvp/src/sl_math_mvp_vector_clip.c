                                        ...
                                        size_t num_elements)
{
  ...
  sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R2, low, low);
  sli_mvp_prog_set_reg_f16c(p->p, SLI_MVP_R3, high, high);
  ...
  sli_mvp_pb_begin_loop(p, rows, &status); {
    sli_mvp_pb_begin_loop(p, cols, &status); {
      sli_mvp_pb_compute(p,
                         ...
                         SLI_MVP_STORE(SLI_MVP_R4, vector_z, SLI_MVP_INCRDIM_WIDTH),
                         &status);
    }
  ...
  sli_mvp_alu_reg_t r;
  r.reg.value.real = low;
  r.reg.value.imag = low;
  MVP->ALU[2].REGSTATE = r.REGSTATE;
  r.reg.value.real = high;
  r.reg.value.imag = high;
  MVP->ALU[3].REGSTATE = r.REGSTATE;
  ...
  MVP->INSTR[0].CFG1 = SLI_MVP_LOAD(0, SLI_MVP_R1, SLI_MVP_ARRAY(0), SLI_MVP_INCRDIM_WIDTH)
                       | SLI_MVP_STORE(SLI_MVP_R4, SLI_MVP_ARRAY(1), SLI_MVP_INCRDIM_WIDTH);
  MVP->INSTR[0].CFG2 = (SLI_MVP_OP(CLIP2A) << _MVP_INSTRCFG2_ALUOP_SHIFT)
  ...
  if (len_remainder > 0) {
    ...
    MVP->ARRAY[2].DIM0CFG = MVP->ARRAY[3].DIM0CFG =
      //Note: The remainder can be any size, cannot utilize len_parallel execution.
    ...
    MVP->INSTR[1].CFG0 = SLI_MVP_ALU_A(SLI_MVP_R1)
                        | SLI_MVP_ALU_X(SLI_MVP_R2)
                        | SLI_MVP_ALU_Y(SLI_MVP_R3)
                        | SLI_MVP_ALU_Z(SLI_MVP_R4);
    MVP->INSTR[1].CFG1 = SLI_MVP_LOAD(0, SLI_MVP_R1, SLI_MVP_ARRAY(2), SLI_MVP_INCRDIM_WIDTH)
                        | SLI_MVP_STORE(SLI_MVP_R4, SLI_MVP_ARRAY(3), SLI_MVP_INCRDIM_WIDTH);
    MVP->INSTR[1].CFG2 = (SLI_MVP_OP(CLIP2A) << _MVP_INSTRCFG2_ALUOP_SHIFT)
