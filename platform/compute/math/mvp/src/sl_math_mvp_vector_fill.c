                                        ...
                                        size_t num_elements)
{
  ...
  const int vector_z1 = SLI_MVP_ARRAY(1);
  ...
  MVP->ARRAY[0].ADDRCFG = (sli_mvp_addr_reg_t)output;
  ...
  if (len_remainder > 0) {
    ...
    MVP->ARRAY[1].ADDRCFG = (sli_mvp_addr_reg_t)&output[ofs_remainder];
    ...
    MVP->INSTR[1].CFG1 = SLI_MVP_STORE(SLI_MVP_R1, SLI_MVP_ARRAY(1), SLI_MVP_INCRDIM_WIDTH);
    MVP->INSTR[1].CFG2 = (SLI_MVP_OP(NOOP) << _MVP_INSTRCFG2_ALUOP_SHIFT)
