                                                     ...
                                                     size_t num_elements)
{
  ...
  sli_mvp_pb_begin_loop(p, rows, &status); {
    sli_mvp_pb_begin_loop(p, cols, &status); {
      sli_mvp_pb_compute(p,
                         SLI_MVP_OP(COPY),
                         SLI_MVP_ALUIN_A(SLI_MVP_R0, SLI_MVP_ALUIN_IMAGNEGATE)
  ...
  if (len_remainder > 0) {
    sli_mvp_pb_config_vector(p->p, vector_x1, (void *)&input[ofs_remainder * 2], data_type, len_remainder, &status);
    sli_mvp_pb_config_vector(p->p, vector_z1, (void *)&output[ofs_remainder * 2], data_type, len_remainder, &status);
  ...
  // Program instruction.
  MVP->INSTR[0].CFG0 = SLI_MVP_ALUIN_A(SLI_MVP_R0, SLI_MVP_ALUIN_IMAGNEGATE)
                       | SLI_MVP_ALU_Z(SLI_MVP_R1);
  ...
  if (len_remainder > 0) {
    ...
    MVP->ARRAY[3].ADDRCFG = (sli_mvp_addr_reg_t)&output[ofs_remainder * 2];

    // Program loop controllers.
    MVP->LOOP[2].CFG = (len_remainder - 1) << _MVP_LOOPCFG_NUMITERS_SHIFT;

    // Program instructions.
    MVP->INSTR[1].CFG0 = SLI_MVP_ALUIN_A(SLI_MVP_R0, SLI_MVP_ALUIN_IMAGNEGATE)
                         | SLI_MVP_ALU_Z(SLI_MVP_R1);
