    ...
    for (int out_y_min = 0, out_y_max; out_y_min < output_height; out_y_min = out_y_max + 1) {
      ...
      for(int out_y_offset = 0; out_y_offset < out_y_num_offsets; ++out_y_offset) {
        ...
        for (int batch = 0; batch < batches; ++batch) {
          ...
          sli_mvp_begin_loop(p, output_depth, &status);
            ...
            sli_mvp_begin_loop(p, output_width_truncated, &status);
              ...
              sli_mvp_postloop_incr_dim(p, SLI_MVP_ARRAY(4), SLI_MVP_INCRDIM_ROW);
              if (!single_out_x) {
                sli_mvp_postloop_incr_dim(p, SLI_MVP_ARRAY(0), SLI_MVP_INCRDIM_VEC);
              }
            sli_mvp_end_loop(p);  // output_width_truncated
            sli_mvp_postloop_incr_dim(p, SLI_MVP_ARRAY(4), SLI_MVP_INCRDIM_COL);
            ...
            sli_mvp_postloop_incr_dim(p, SLI_MVP_ARRAY(3), SLI_MVP_INCRDIM_COL);

          sli_mvp_end_loop(p);  // output_depth
